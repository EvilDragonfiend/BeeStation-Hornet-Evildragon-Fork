...
/datum/antagonist/proc/antag_listing_name()
	...
	if(owner.current)
		...
		return "<a href='byond://?[HREF_TYPE(var_edit)][HREF_PARAM(var_edit::Vars, REF(owner))][HrefToken()]'>[owner.name]</a> "
...
/datum/antagonist/proc/antag_listing_commands()
	if(!owner)
		return
	var/list/parts = list()
	parts += "<a href='byond://?[HREF_TYPE(admin_pm)][HREF_PARAM(admin_pm::msg_target, ckey(owner.key))]'>PM</a>"
