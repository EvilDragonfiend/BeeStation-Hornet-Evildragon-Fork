...
/proc/reopen_roundstart_suicide_roles()
	...
	valid_positions += SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SUPPLY)
	valid_positions += SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SERVICE)
...
/datum/game_mode/proc/generate_credit_text()
	...
	len_before_addition = round_credits.len
	for(var/datum/mind/current in SSticker.mode.get_all_by_department(SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SUPPLY)))
		custom_title_holder = get_custom_title_from_id(current, newline=TRUE)
		round_credits += "<center><h2>[current.name] as the [current.assigned_role][custom_title_holder]</h2>"
	if(round_credits.len == len_before_addition)
	...
	len_before_addition = round_credits.len
	for(var/datum/mind/current in SSticker.mode.get_all_by_department(SSdepartment.get_jobs_by_dept_id(DEPT_NAME_SERVICE))) // gimmicks shouldn't be here, but let's not make the code dirty
